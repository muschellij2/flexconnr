...
def apply_model(image1, image2, image3, pred_model):
    ...
    slice_2d_t2 = np.zeros(dim_2d, dtype=np.float16)
    output_image = np.zeros(dim, dtype=np.float16)

    for k in range(dim[2]):
        slice_2d_t1[0, :, :, 0] = image1[:, :, k]
        slice_2d_fl[0, :, :, 0] = image2[:, :, k]
        slice_2d_t2[0, :, :, 0] = image3[:, :, k]
        pred = pred_model.predict([slice_2d_t1, slice_2d_fl, slice_2d_t2])
...
def py_predict_flexconn(t1, flair, t2, models, outdir, gpu, normalize=True):
    ...
    im1 = os.path.abspath(os.path.expanduser(t1))
    im2 = os.path.abspath(os.path.expanduser(flair))
    im3 = os.path.abspath(os.path.expanduser(t2))
    
    outdir = os.path.abspath(os.path.expanduser(outdir))
    _, base, _ = split_filename(im1)
    outname1 = os.path.join(outdir, base + "_LesionMembership.nii.gz")
    outname2 = os.path.join(outdir, base + "_LesionMask.nii.gz")
    ...
    for t in range(len(models)):
        ...
        mem = apply_model(vol1, vol2, vol3, model)
        outvol[:, :, :, t] = mem
